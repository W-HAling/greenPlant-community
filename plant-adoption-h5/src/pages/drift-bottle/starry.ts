...
export function createStarryEngine(canvasId: string, options: any = {}) {
    ...
    function checkPickable(){
      ...
      if(options.onPickableChange) options.onPickableChange(has);
      return has;
    }
    ...
    function doPick(bottle){
      ...
      pickingAnim={x:bottle.x,y:bottle.y,t:0};
    }
    ...
    var onClick = function(e){
      ...
      if(bottle) {
        if(options.onPickClick) options.onPickClick(bottle);
      } else {
        if(options.onPickCancel) options.onPickCancel();
      }
    };
    ...
    return {
      ...
      setPickMode: function(mode: boolean) {
      ...
      playPickAnim: function(bottleRef) {
        doPick(bottleRef);
      },
      toggleAudio: function(forceState?: boolean) {
        if(!aCtx)initAudio();if(aCtx.state==='suspended')aCtx.resume();
        aOn = forceState !== undefined ? forceState : !aOn;
        aGain.gain.linearRampToValueAtTime(aOn?.12:0,aCtx.currentTime+.3);
        return aOn;
      },
